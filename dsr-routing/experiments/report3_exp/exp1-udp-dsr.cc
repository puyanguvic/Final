...
int main (int argc, char *argv[])
{
  ...
  app->Setup (udpSocket, sinkAddress, 200, 50000, DataRate ("13Mbps"), budget, false);
  // app->Setup (udpSocket, sinkAddress, 200, 500, DataRate ("1Mbps"), false);
  nodes.Get (0)->AddApplication (app);
  app->SetStartTime (Seconds (1.));
  app->SetStopTime (Seconds (20.));

  AnimationInterface anim("exp1-dsr-udp.xml");
  anim.SetConstantPosition (nodes.Get(0), 0.0, 100.0);
  anim.SetConstantPosition (nodes.Get(1), 100.0, 0.0);
  anim.SetConstantPosition (nodes.Get(2), 100.0, 200.0);
