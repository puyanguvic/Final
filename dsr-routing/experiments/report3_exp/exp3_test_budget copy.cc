                        ...
                        uint16_t dataRate, bool flag)
{
  Ptr<Socket> dsrSocket = Socket::CreateSocket (node, UdpSocketFactory::GetTypeId ());
  Ptr<DsrUdpApplication> dsrApp = CreateObject<DsrUdpApplication> ();
  dsrApp->Setup (dsrSocket, sinkAddress, packetSize, nPacket, DataRate(std::to_string(dataRate) + "Mbps"), 0,flag);
  node->AddApplication (dsrApp);
  dsrApp->SetStartTime (Seconds (startTime));
  dsrApp->SetStopTime (Seconds (stopTime));
}
...
main (int argc, char *argv[])
{
  ...
  std::string qID1 = "n0n3-n0";
  Simulator::ScheduleNow (&CheckQueueSize, qdisc[0].Get (0), qID1);
  std::string qID2 = "n0n1-n0";
  Simulator::ScheduleNow (&CheckQueueSize, qdisc[2].Get (0), qID2);
  // std::string qID3 = "n2n5-n2";
  // Simulator::ScheduleNow (&CheckQueueSize, qdisc[10].Get (0), qID3);
  ...
  for (int i=1; i<11; i++)
  {
    InstallDGPacketSend (nodes.Get(2), sinkAddress, (i-1)*0.5, i*0.5, PacketSize, NPacket, budget, 2, true);
  }
...
// ------------------------ Network DSR TCP application--------------------------------------------
  ...
  InstallPacketSink (nodes.Get (8), sinkPort, "ns3::UdpSocketFactory", BeginTime, StopTime);
  ...
  uint32_t budget1 = 20;
  // InstallDGPacketSend (nodes.Get(0), sinkAddress, BeginTime, StopTime, PacketSize, NPacket1, budget1, 2, false);
  // InstallDGPacketSend (nodes.Get(1), sinkAddress_2, BeginTime, StopTime, PacketSize, NPacket1, budget1, 2, false);
  // InstallDGPacketSend (nodes.Get(2), sinkAddress_3, BeginTime, StopTime, PacketSize, NPacket1, budget1, 2, false);
   for (int i=1; i<11; i++)
  {
